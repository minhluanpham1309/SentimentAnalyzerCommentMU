...
dz.addEventListener('drop',e=>{e.preventDefault();dz.style.borderColor='var(--le-border)';_load(e.dataTransfer.files[0]);});

function _load(file){if(!file)return;const r=new FileReader();r.onload=e=>{try{_d=_pcsv(e.target.result);_d.forEach((row,i)=>{row._i=i;row._o=row.sentiment;});_e.clear();_flt='all';_q='';_pg=1;_sel=-1;document.getElementById('le-fname').textContent=file.name;document.getElementById('le-up').style.display='none';document.getElementById('le-ed').style.display='flex';_apply();_toast(`✅ Đã load ${_d.length.toLocaleString()} comments`);}catch(err){_toast('❌ Lỗi: '+err.message);}};r.readAsText(file,'UTF-8');}
function le_new(){document.getElementById('le-up').style.display='flex';document.getElementById('le-ed').style.display='none';document.getElementById('le-fi').value='';}
...
function _sl(i,label,gi){_d[i].sentiment=label;if(label!==_d[i]._o)_e.add(i);else _e.delete(i);_apply();_sel=gi;_rt();const t=document.querySelector(`#le-tb tr[data-gi="${gi}"]`);if(t)t.classList.add('le-sel');}
